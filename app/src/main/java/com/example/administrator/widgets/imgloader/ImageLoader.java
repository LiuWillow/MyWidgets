...
public class ImageLoader
{
   ...
   //初始化后台轮询线程
                          ...
                          final boolean isFromNet)
    {
        ...
        if (bm != null)
        {
            refreashBitmap(urlString, imageView, bm);
        } else
        {
            addTask(buildTask(urlString, imageView, isFromNet));
        }
        ...
        return new Runnable()
        {
            ...
            public void run()
            {
                ...
                if (isFromNet)
                {
                    ...
                    {
                        Log.e(TAG, "find image :" + urlString + " in disk cache .");
                    ...
                    } else
                    {
                        if (isDiskCacheEnable)// 检测是否开启硬盘缓存
                        {
                            boolean downloadState = DownloadImgUtils
                                    .downloadImgByUrl(urlString, file);
                            if (downloadState)// 如果下载成功
                            {
                                Log.e(TAG,
                                        "download image :" + urlString
                        ...
                        // 直接从网络加载
                        {
                            Log.e(TAG, "load image :" + urlString + " to memory.");
                            bm = DownloadImgUtils.downloadImgByUrl(urlString,
                                    imageView);
                        }
                    }
                } else
                {
                    bm = loadImageFromLocal(urlString, imageView);
                }
                // 3、把图片加入到缓存
                addBitmapToLruCache(urlString, bm);
                refreashBitmap(urlString, imageView, bm);
